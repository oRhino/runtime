...
//DisguisedPtr<objc_object> 哈希函数, size_t,引用计数
...
/*
SideTable 这个类，它用于管理引用计数表和 weak 表，并使用 spinlock_lock 自旋锁来防止操作表结构时可能的竞态条件。它用一个 64*128 大小的 uint8_t 静态数组作为 buffer 来保存所有的 SideTable 实例
     */
struct SideTable {
    //保证原子操作的自旋锁
    spinlock_t slock;
 ...
 *
 * @param location Address of __weak ptr.  弱引用对象的地址
 * @param newObj Object ptr.  被修饰的对象
 */
id
objc_initWeak(id *location, id newObj)
{
    ...
    //模板函数  老对象,新对象,dealoc的过程中是否可以crash
...
#   define POOL_BOUNDARY nil
    ...
    //返回栈的起始位置(起始为AutoreleasePoolPage的所有成员变量+)
    id * begin() {
    ...
    void kill() 
    {
        ...
        //记录当前的page,获取page的parent,断开连接,删除page,直到page为this
    ...
    /*
     ...
     result = 0x100816000
     */
    //fastCheck() 用来检查当前的 result 是不是一个AutoreleasePoolPage。
    ...
    //获取当前正在使用的page,如果不满就添加这个对象,满了就执行新建page的操作,不存在page就执行autoreleaseNoPage
    ...
    //从传入的page开始遍历整个双向链表，直到：查找到一个未满的 AutoreleasePoolPage,如果没有则使用构造器传入parent 创建一个新的 AutoreleasePoolPage
    //查找到一个可以使用的AutoreleasePoolPage之后，会将该页面标记成hotPage，然后调动page->add方法添加对象。
    ...
    id *autoreleaseFullPage(id obj, AutoreleasePoolPage *page)
    {
        ...
        //向下遍历 直到找到一个没有full的AutoreleasePoolPage,如果还是没有,就新建一个,
        //设置这个为正在使用的hotPage,添加目标元素
        do {
    ...
    //当前内存中不存在AutoreleasePoolPage，就要从头开始构建自动释放池的双向链表
    static __attribute__((noinline))
...
public:
    ...
    //调用objc_autoreleasePoolPush 的时候，都会把一个POOL_BOUNDARY push到自动释放池的栈顶，并且返回这个 POOL_BOUNDARY哨兵对象。
    ...
    //1.使用 pageForPointer 获取当前token所在的 AutoreleasePoolPage,
    //2.调用releaseUntil方法释放栈中的对象，直到 stop,
...
/*
 size_t 存储引用计数 64位
 第一位 weakly_referenced (0 代表有t弱引用计数,1没有)
 第二位 deallocating 是否正在dealloc
 后面的 RC引用计数
 所以需要向右偏移两位
 SIDE_TABLE_RC_ONE
 */

//通过当前对象的指针从SideTables(hash查找)获取SideTable
//通过hash查找得到引用计数
//进行位移进行引用计数的相加(64位存储,前两位存储的是别的标志,并不是直接+1)
id
...
//引用计数
//它所返回的引用计数只是某个给定时间点上的值，该方法并未考虑到系统稍后会把自动释放吃池清空，因而不会将后续的释放操作从返回值里减去。clang 会尽可能把 NSString 实现成单例对象，其引用计数会很大。如果使用了 TaggedPointer，NSNumber 的内容有可能就不再放到堆中，而是直接写在宽敞的64位栈指针值里。其看上去和真正的 NSNumber 对象一样，只是使用 TaggedPointer 优化了下，但其引用计数可能不准确。
uintptr_t
objc_object::sidetable_retainCount()
{
    //先从SideTable的静态方法获取当前实例对应的 SideTable 对象，其 refcnts 属性就是之前说的存储引用计数的散列表，这里将其类型简写为 RefcountMap：
//    typedef objc::DenseMap<disguisedptr<objc_object>,size_t,true> RefcountMap;</disguisedptr<objc_object>
...
{
    //fastpath的含义是，为1的概率大，slowpath的含义是为0的概率大。
    if (slowpath(checkNil && !cls)) return nil;

#if __OBJC2__
    //hasCustomAWZ判断当前class是否自己实现alloc/allocWithZone方法
    
    if (fastpath(!cls->ISA()->hasCustomAWZ())) {
       
        //说明没有自己实现alloc/allocWithZone方法
        // No alloc/allocWithZone implementation. Go straight to the allocator.
        // fixme store hasCustomAWZ in the non-meta class and 
        // add it to canAllocFast's summary
        
        //判断当前的class是否支持快速alloc
        if (fastpath(cls->canAllocFast())) {
            // No ctors, raw isa, etc. Go straight to the metal.
            
           // 如果可以，直接调用calloc函数，申请内存空间.如果创建失败，也会调用callBadAllocHandler函数。如果创建成功，就去初始化isa指针和dtor。
            ////查看一下类是否有析构函数
            ...
            //如果当前的class不支持快速alloc，调用class_createInstance(cls, 0);方法去创建一个新的对象。
    ...
    //实现了自定义的allocwithzone 且allocWithZone = true
...
//isMemberOfClass判断对象是否是目标类的实例对象
- (BOOL)isMemberOfClass:(Class)cls {
...
//isKindOfClass判断对象是否是目标类的实例对象,或者是目标类的子类
- (BOOL)isKindOfClass:(Class)cls {
