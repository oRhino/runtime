...
////查找到Category的方法列表、属性列表、协议列表，然后通过对应的attachLists函数，添加到Class对应的class_rw_t结构体中。
static void 
attachCategories(Class cls, category_list *cats, bool flush_caches)
{
    ...
    while (i--) {
        ...
        if (mlist) {
            //最后编译的分类的方法最先被添加到分类方法数组中
            mlists[mcount++] = mlist;
            fromBundle |= entry.hi->isBundle();
            // 执行添加操作
            //在有多个Category和原类的方法重复定义的时候，原类和所有Category的方法都会存在，并不会被后面的覆盖。假设有一个方法叫做method，Category和原类的方法都会被添加到方法列表中，只是存在的顺序不同。
            //在进行方法调用的时候，会优先遍历Category的方法，并且后面被编译的Category，会被优先调用。上面的例子调用顺序就是Category3 -> Category2 -> Category1 -> TestObject。如果从方法列表中找到方法后，就不会继续向后查找，这就是类方法被Category”覆盖”的原因。
        }
...
//# 加载分类 categories
    ...
    for (EACH_HEADER) {
        ...
        //循环会遍历取出的Category数组，将每个category_t对象取出，最终执行addUnattachedCategoryForClass函数添加到Category哈希表中。
        for (i = 0; i < count; i++) {
...
getMethod_nolock(Class cls, SEL sel)
{
    ...
    assert(cls->isRealized());
    //getMethodNoSuper_nolock方法,如果当前类没有,将cls赋值为父类,继续从父类里面查找
...
**********************************************************************/
//class_getInstanceMethod获取的method可能是在本身的class中获取，或者是在superclass中获取
...
addMethod(Class cls, SEL name, IMP imp, const char *types, bool replace)
{
    ...
    method_t *m;
    ////关键方法:getMethodNoSuper_nolock只在本类中查找是否含有方法，而不在父类中查找
    ////可以从本类中查找方法
    if ((m = getMethodNoSuper_nolock(cls, name))) {
        ...
        ////没有从本类中查找到方法
        // fixme optimize
        ////在class中添加method，而method的结构体系：class->method_array_t(methods)->method_list_t->method
...
{   //// 如果cls为nil，直接返回NO，添加方法失败
    ...
    mutex_locker_t lock(runtimeLock);
    //// 实现函数,最后一个参数NO
    //// class_replaceMehod,最后一个参数是YES
    ////返回值取反,IMP
    ////将要添加的method，在cls中存在，并且已经有了函数指针那么添加方法失败。
