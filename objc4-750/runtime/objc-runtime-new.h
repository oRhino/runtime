...
/*********消息传递的过程************/
//*******方法的查找过程
//1(缓存是否命中).在方法缓存中查找:给定值是SEL,目标值是对应的buckt_t的IMP


//2(当前类方法列表是否命中).当前类中查找
//对于已经排序好的列表,采用二分查找算法查找方法对应执行函数
//对于没有排序的列表,采用一般遍历查找方法对应执行函数

//3(逐级父类方法列表是否命中).父类逐级查找
//curClass = curClass->superclass,curClass != nil,从方法缓存中查找->方法列表,遍历
...
/**
 函数四要素:名称 (name),返回值.参数(types),函数体(imp)
...
//protocol_t 继承 objc_object,所以具有对象的特征
...
struct objc_class : objc_object {
    ...
    //成员变量,属性,方法都在这里面,主要是对class_rw_t的封装
