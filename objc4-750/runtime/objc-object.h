...
objc_object::rootDealloc()
{
    ...
    //判断是否可以直接释放
    //1.使用了非指针型的isa  isa.nonpointer
    //2.是否有weak指针指向它.
    //3.是否有关联对象.
    //4.内部实现是否有涉及c++的内容,以及当前对象是否使用ARC来管理内存,如果有返回yes
    //5.是否使用引用计数表来管理内存,isa.nonpointer直接存储的引用信息.
