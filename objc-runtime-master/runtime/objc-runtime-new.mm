                 ...
                 int flags)
{
    ...
    auto rwe = cls->data()->extAllocIfNeeded();//rwe
...
static Class realizeClassWithoutSwift(Class cls, Class previously)
{
    ...
    if (ro->flags & RO_FUTURE) {
        ...
        rw = objc::zalloc<class_rw_t>();//rw初始化
        rw->set_ro(ro); //设置ro
        rw->flags = RW_REALIZED|RW_REALIZING|isMeta;//设置flag
        cls->setData(rw);
    }
...
static void load_categories_nolock(header_info *hi) {
    ...
    auto processCatlist = [&](category_t * const *catlist) {
        for (unsigned i = 0; i < count; i++) {
           ...
           const char *str = cls->mangledName();
...
load_images(const char *path __unused, const struct mach_header *mh)
{
    if (!didInitialAttachCategories && didCallDyldNotifyRegister) {
        didInitialAttachCategories = true; //只会执行一次
                              ...
                              size_t *outAllocatedSize = nil)
{
    ...
    // Read class's info bits all at once for performance
    bool hasCxxCtor = cxxConstruct && cls->hasCxxCtor();//c++构造函数
    bool hasCxxDtor = cls->hasCxxDtor(); //c++析构
    bool fast = cls->canAllocNonpointer(); //是否可以是NonPointer
    size_t size; //局部变量 需要开辟的内存大小
