...
//template<typename T> class StripedMap 从数据结构角度看的话，它是作为一个 Key 是 void * Value 是 T 的 hash 表来用的。
//T 作为 SideTable 和 spinlock_t 类型使用。
//抽象类型 T 必须支持 lock、unlock、forceReset、lockdebug_lock_precedes_lock 函数接口
template<typename T>
class StripedMap {
 ...
 public:
    ...
    const T& operator[] (const void *p) const {
        // 这里 const_cast<StripedMap<T>>(this) 有必要吗，本来就是读取值，并不会修改 StripedMap 的内容
...
//DisguisedPtr 类似于指针类型 T *，只是存储的值被伪装成对诸如 “leaks” 之类的工具隐藏。nil 本身是伪装的，因此 0 值的内存可以按预期工作，让 nil 指针像 non-nil 指针那样正常运行它的操作，而不会让程序崩溃。这意味着 0x80..00 本身也伪装了，但我们不在乎。请注意，weak_entry_t 知道此编码。

//模版工具类，主要的功能是把 T 指针（T 类型变量的地址）转化为一个 unsigned long，实现指针到整数的相互映射，起到指针伪装的作用，使指针隐藏于系统工具（如 leaks 工具）。
template <typename T>
class DisguisedPtr {
