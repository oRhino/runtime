...
enum {
    ...
    INIT_CACHE_SIZE      = (1 << INIT_CACHE_SIZE_LOG2), //初始容量 4
    MAX_CACHE_SIZE_LOG2  = 16,
    MAX_CACHE_SIZE       = (1 << MAX_CACHE_SIZE_LOG2), //最大容量 2>16
};
...
void cache_t::reallocate(mask_t oldCapacity, mask_t newCapacity, bool freeOld)
{
    ...
    //储存新的buckets,和mask(newCapacity - 1)
...
#pragma mark - 缓存添加一条内容
//1.计算当前已占用多少容量
//2.读取旧容量,判断是否需要扩容
//2.1 如果是没有开辟缓存空间,开辟新空间 1<<2,一次开辟4个
//2.2 如果已占用数量小于当前容量的3/4,不需要扩容
//2.3 如果超过3/4容量,进行扩容,扩容一倍
ALWAYS_INLINE
void cache_t::insert(Class cls, SEL sel, IMP imp, id receiver)
{
    ...
    mask_t newOccupied = occupied() + 1; //当前缓存需要占用的数量
    ...
    if (slowpath(isConstantEmptyCache())) {
        ...
        if (capacity > MAX_CACHE_SIZE) {
            capacity = MAX_CACHE_SIZE; //最大容量 1<<16
        }
        //扩容
    ...
    bucket_t *b = buckets();
    mask_t m = capacity - 1;
    mask_t begin = cache_hash(sel, m); //哈希函数 (mask_t)(uintptr_t)sel & mask
    mask_t i = begin;
    ...
    do {
        if (fastpath(b[i].sel() == 0)) {
            //插入位置没有冲突
            incrementOccupied(); // _occupied++,已占用+1
            b[i].set<Atomic, Encoded>(sel, imp, cls); //添加缓存
            return;
        }
        if (b[i].sel() == sel) {
