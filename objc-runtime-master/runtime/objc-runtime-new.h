...
//数据容器,有自己的迭代器用于遍历所有元素
struct entsize_list_tt {
    uint32_t entsizeAndFlags; //存储元素的大小和flag
    uint32_t count; //容量
    ...
    //根据索引返回指定元素的引用,i可以等于count,即可以返回最后一个元素的后面
    ...
    //迭代器
    struct iterator {
        uint32_t entsize; //每个元素的大小
        uint32_t index;  // keeping track of this saves a divide in operator- //当前迭代器的索引
...
struct method_t {
    ...
    // and implementation.
    //WWDC2020优化  原来采用3*8 = 24字节来存储
    //small只需要12字节
    struct big {
        SEL name; ///方法编号
        const char *types; //方法签名
        MethodListIMP imp; //函数指针 实现
    };
 ...
 public:
    ...
    void attachLists(List* const * addedLists, uint32_t addedCount) {
        ...
        if (hasArray()) {
            ...
            uint32_t oldCount = array()->count; //之前的长度
            uint32_t newCount = oldCount + addedCount;
            //扩容生成新的数组
            array_t *newArray = (array_t *)malloc(array_t::byteSize(newCount));
            newArray->count = newCount; //更新长度
            array()->count = newCount;

            //添加原有的(添加的位置是在后面 i + addedCount)
...
#pragma mark - rw wwdc2020优化/可能没有,减少内存
