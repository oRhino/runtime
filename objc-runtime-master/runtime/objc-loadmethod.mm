...
**********************************************************************/
#pragma mark - 添加分类中的load方法到全局分类的load方法表中
...
**********************************************************************/
#pragma mark - 调用类的load方法
static void call_class_loads(void)
{
    int i;
    
    // Detach current loadable list.
    struct loadable_class *classes = loadable_classes;//load 方法列表[cls,load的imp]
    int used = loadable_classes_used; //全局变量 个数
...
static bool call_category_loads(void)
{
    ...
    // Detach current loadable list.
    struct loadable_category *cats = loadable_categories; //全局分类load列表[cat,IMP]
    ...
    for (i = 0; i < used; i++) {
        ...
        if (cls  &&  cls->isLoadable()) { //判断类是否load
            //类的load方法要优先于类别
            if (PrintLoading) {
